...
namespace بذرة_أمل
{
	public partial class Form15 : Form
	{
		...
		public Form15(string purchase_price_that_was, string rec_id)
		{
			InitializeComponent();
		...
		public void getAllRecords()
		{
			...
			String selectSQL = "SELECT * FROM CPAYMENT_PURCHASE where crec_id=" + rec_id + " order by cpaym_id desc";
			...
			while (reader.Read()) {
				...
				dataGridView2.Rows.Add(new string[] {
					reader["cpaym_id"].ToString(),
					reader["crec_id"].ToString(),
					reader["cpayment"].ToString(),
					DateTime.Parse(reader["cpayment_date"].ToString()).ToString("dd/MM/yyyy"),
						
				});
				
				
				all_payment += double.Parse(reader["cpayment"].ToString());
		...
		void Button3Click(object sender, EventArgs e)
		{
			if (!textBox8.Text.Trim().Equals("") && !textBox8.Text.Trim().Equals("0")) {
				...
				String insertSQL = "insert into CPAYMENT_PURCHASE (crec_id,cpayment,cpayment_date) values (" + rec_id + "," + textBox8.Text + ",'" + dateTimePicker3.Value.ToShortDateString() + "')";
		...
		void Button2Click(object sender, EventArgs e)
		{
			if (dataGridView2.SelectedRows.Count > 0) { 
				...
				if (!string.IsNullOrEmpty(textBox8.Text) && !textBox8.Text.Trim().Equals("0")) {
					...
					String insertSQL = "update CPAYMENT_PURCHASE set cpayment=" + textBox8.Text + ",cpayment_date='" + dateTimePicker3.Value.ToShortDateString() + "' where cpaym_id=" + dataGridView2.SelectedRows[0].Cells[0].Value.ToString();
		...
		public void delete(string id)
		{
			...
			String insertSQL = "DELETE FROM CPAYMENT_PURCHASE where cpaym_id=" + id;
